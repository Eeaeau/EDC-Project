...
def partition_data_set(data_set, data_labels, num_clusters = 64, use_tensor=False):
    ...
    if use_tensor:
        # data
        dataset_tensor = torch.from_numpy(data_set)

        # kmeans
        cluster_ids_x, cluster_centers = kmeans(
        X=dataset_tensor, num_clusters=num_clusters, distance='euclidean', tol = 1000, device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu")) 
...
def plot_image(dataset, data_labels, start_index, end_index):
    ...
    for i in range(image_count):
        axs[row, col].imshow(dataset[i].reshape(28,28), interpolation='none')

        axs[row, col].set_title("Number "+str(data_labels[i][0]))
